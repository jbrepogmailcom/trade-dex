...
from trust_wallet_codes import get_keys

wallet_address, private_key = get_keys()
...
TOKENS = {
    ...
    # Example MPS (0 decimals). If your actual MPS token is at a different address, update it:
    "MPS":   Web3.to_checksum_address("0xfa57aa7beed63d03aaf85ffd1753f5f6242588fb"),
}
...
def main():
    """
    Usage:
        python3 trade.py <SELL_AMOUNT_OR_?> <SELL_TOKEN> <BUY_AMOUNT_OR_?> <BUY_TOKEN> [last_purchase_price] [--repeat_times=n]
    ...
    if len(sys.argv) not in [5, 6, 7]:
        print("Usage: python3 trade.py <sell_amount_or_?> <sell_token> <buy_amount_or_?> <buy_token> [last_purchase_price] [--repeat_times=n]")
        print("Example exact-sell: python3 trade.py 12 USDC ? MPS 0.98")
        print("Example exact-buy:  python3 trade.py ? USDC 1 MPS 1.05")
        sys.exit(1)
    ...
    last_purchase_price = float(sys.argv[5]) if len(sys.argv) >= 6 and not sys.argv[5].startswith("--") else None
    repeat_times = 1

    if len(sys.argv) == 7 or (len(sys.argv) == 6 and sys.argv[5].startswith("--")):
        repeat_arg = sys.argv[6] if len(sys.argv) == 7 else sys.argv[5]
        if repeat_arg.startswith("--repeat_times="):
            repeat_times = int(repeat_arg.split("=")[1])
